const CircleText = ({ diameter = 300, color = 'var(--color-placeholder)', children }) => {
  ...
  return (
    <div style={{ width: diameter, height: diameter, fontSize: fontSize }}>
      <svg viewBox={viewBox}>
        <defs>
          <path id="circlePath" d={`M ${a},${b} a ${c},${c} 0 0,1 0,-${d} a ${c},${c} 0 0,1 0,${d} Z`} transform={`rotate(0,${a},${a})`}>
            <animateTransform attributeName="transform" begin="0s" dur="15s" type="rotate" from={`0 ${radius} ${radius}`} to={`360 ${radius} ${radius}`} repeatCount="indefinite" />
          </path>
        </defs>
